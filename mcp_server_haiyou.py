...
async def search_result(alt: str) -> str:
    ...
    return f"![alt]({img_url})"
@mcp.tool()
async def run_wellloginterpretation(query: str,file_name:str) -> str:
    """
    运行测井解释程序，输出计算结果
    参数:
        query: 工作路径
    ...
    # except RuntimeError:
    #     # 如果此时不在 Flask app context，可以手动拼 URL（假设运行在 http://localhost:5000）
    table ="""| 参数 | 孔隙度 | 石英 | 长石（斜长石和中钾长石） | 其他 | 波速 | 密度 |
|---|---:|---:|---:|---:|---:|---:|
| 值 | 14.5% | 64.13% | 18.81% | 2.57% | 4.28 km/s | 2.355 |
"""
    return f"![测井解释结果]:{table}"
@mcp.tool()
async def run_GenerateModel(query: str,file_name:str) -> str:
    """
    运行模型生成程序，输出建模结果
    参数:
        query: 工作路径
    ...
    try:
        ...
        # 如果此时不在 Flask app context，可以手动拼 URL（假设运行在 http://localhost:5000）
        img_url1 = "http://localhost:5000/static/simulation/ai建模1.jpg"
        img_url2 = "http://localhost:5000/static/simulation/ai建模2.png"
    return f"![模型生成结果为InputModel.inp],示意图为({img_url1}，{img_url2})"
@mcp.tool()
async def run_ParameterSVCalc(query: str,file_name:str) -> str:
    """
    运行模型波速计算程序，输出计算结果
    参数:
        query: 工作路径
    ...
    # except RuntimeError:
        # 如果此时不在 Flask app context，可以手动拼 URL（假设运行在 http://localhost:5000）
    img_url1 = "http://localhost:5000/static/simulation/波速1.jpg"
    img_url2 = "http://localhost:5000/static/simulation/波速2.png"
    return f"![模型参数计算结果]({img_url1},{img_url2})"
@mcp.tool()
async def run_ParameterECalc(query: str,file_name:str) -> str:
    """
    运行模型弹性模量计算程序，输出计算结果
    参数:
        query: 工作路径
    ...
    # except RuntimeError:
        # 如果此时不在 Flask app context，可以手动拼 URL（假设运行在 http://localhost:5000）
    img_url = "http://localhost:5000/static/simulation/弹性模量.gif"
    return f"![模型参数计算结果]({img_url})"

@mcp.tool()
async def run_makereport(query: str,) -> str:
    """
    运行报告编制程序，输出计算结果
    参数:
        query: 工作路径
