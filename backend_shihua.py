...
TOOLS = [
    ...
    # },
    {
        ...
        "function": {
            "name": "run_map2petrel",
            "description": "从高程数据建立Petrel地质模型。注意：此工具需要用户提供详细的网格参数（如rnx, rny, rnz）和坐标范围（xmin, xmax等）。如果用户未提供，请询问用户。",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
                    ...
                    "well_file": {
                        "type": "string",
                        "description": "井轨迹文件名 (默认: welllog.txt)"
                    },
                    "rnx": {
                        "type": "integer",
                        "description": "X方向网格数 (默认: 110)"
                    },
                    "rny": {
                        "type": "integer",
                        "description": "Y方向网格数 (默认: 110)"
                    },
                    "rnz": {
                        "type": "integer",
                        "description": "Z方向网格数 (默认: 100)"
                    },
                    "xmin": {
                        "type": "number",
                        "description": "X方向最小值"
...
SYSTEM_PROMPT   = (
    ...
    "你需要对用户的提示操作步骤：step1.提示用户先设定工作路径；step2.提示用户将高程数据map.txt人工放到工作目录下面，用户回复后，执行从高程数据建立Petrel地质模型,提示用户建模完成；step3.提示用户将测井数据放到工作目录后，可以运行测井地应力计算，反馈运行结果；step4.提示用户可以进行Aifrac仿真模拟，询问用户是否进行地应力和压裂仿真模拟，用户确认，反馈运行情况；step5.如果用户是否需要查看其他信息（如井周压裂模拟结果、四维压裂模拟结果、地应力数据统计结果），将调取其他数据进行查看。\n"
