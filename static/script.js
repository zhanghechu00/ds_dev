...
async function startNewChat() {
  // 先在本地创建一个空会话
  const id = uid();
  state.conversations.unshift({ id, title: '新建对话', model: modelSelect.value, messages: [] });
  state.activeId = id;
  saveState(); renderSidebar(); renderAll();

  // 请求后端初始化（写入 system + 欢迎语）
  initPromise = fetch('/chat', {
    method: 'POST',
    ...
    convo.messages.push({ role: 'assistant', content: reply || '(空)' });
    saveState(); renderAll();
  }).catch(err => {
    const convo = getActive();
    convo.messages.push({ role: 'assistant', content: `初始化失败：${err.message}` });
    saveState(); renderAll();
...
function createConversationWithWelcome() {
  const id = uid();
  const convo = {
    id,
    title: '新建对话',
    ...
    messages: [
      { role: 'assistant', content: WELCOME_MD }   // ★ 先由 AI 开场
    ]
  };
...
const WELCOME_MD = [
  "您好！我是Aifrac智能油气专家，很高兴为您服务！",
"我是由武汉大学联合武汉市鸿理科技有限公司开发的Aifrac智能油气专家，能够协助您完成从地质建模到压裂仿真的全流程工作。",
"在我们开始之前，请您先提供整改项目的工作路径！",
].join("\n");
// ====== 渲染：Sidebar / 主区 ======
...
function appendMessage(role, content, toolOutput, needParams) {
  ...
  wrap.innerHTML = `
    <div class="avatar">${role === 'user' ? '🧑' : '<img src="/static/icons/aifrac.png" alt="AI" />'}</div>
    <div class="bubble">
...
if (!state.conversations.length) {
  // 没有任何会话：从后端初始化一个，拿到欢迎语
  startNewChat();
} else {
  // 已有会话：正常渲染
  renderSidebar(); renderAll(); autoResize();
}
