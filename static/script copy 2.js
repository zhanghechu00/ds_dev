...
// ====== 消息渲染 ======
function appendMessage(role, content, toolOutput, needParams,liveLog) {
  ...
  if (needParams && needParams.fields) {
   ...
   // 实时日志控制台（SSE）
  if (!extra && liveLog && liveLog.path) {
    const escPath = encodeURIComponent(liveLog.path);
    extra = `
      <div class="tool-block">
        <div class="log-header">
          <strong>实时日志</strong>
          <span class="muted">${escapeHtml(liveLog.path)}</span>
          <div>
            <button class="action-btn" data-logstop>停止</button>
            <button class="action-btn" data-logclear>清空</button>
          </div>
        </div>
        <pre class="log-console" data-log-src="/logs/stream?path=${escPath}"></pre>
      </div>`;
  }
  wrap.innerHTML = `
  ...
  const logEl = wrap.querySelector('.log-console');
  if (logEl && logEl.dataset.logSrc) {
    const es = new EventSource(logEl.dataset.logSrc);
    es.onmessage = (ev) => {
    ...
    wrap.querySelector('[data-logstop]')?.addEventListener('click', () => {
      es.close();
      wrap.querySelector('[data-logstop]').disabled = true;
    });
    wrap.querySelector('[data-logclear]')?.addEventListener('click', () => {
      logEl.textContent = '';
    });
  }
...
async function sendParamForm(tool, params) {
  ...
  try {
    ...
    const liveLog = data.live_log || null;
    convo.messages[pendingIndex] = { role: 'assistant', content: reply || '(空)', toolOutput , liveLog };
    renderAll();
  } catch (e) {
