...
TOOLS = [
   {
        "type": "function",
        "function": {
            "name": "run_wellloginterpretation",
            "description": "运行测井数据解释",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "run_GenerateModel",
            "description": "运行生成式Ai，生成模型",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "run_ParameterSVCalc",
            "description": "运行模型波速计算",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
        ...
        "type": "function",
        "function": {
            "name": "run_ParameterECalc",
            "description": "运行模型弹性模量计算",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {
...
WELCOME_MD = (
    ...
    "我是由中海油集团联合武汉大学开发的Aifrac智能体，能够协助您完成测井解释、模型生成和参数计算等工作。\n"
    "第一步，请您先提供整个项目的工作路径！\n\n"
)

SYSTEM_PROMPT   = (
    "你是由中海油集团联合武汉大学开发的Aifrac智能体，你能够调用一些工具和解答相关专业问题 。你的回复都非常专业简洁\n"
    "你需要对用户的提示操作步骤：step1.提示用户先设定工作路径；step2.提示用户将测井数据文件，人工放到工作目录下面，用户回复后，执行测井解释,提示用户分析完成，反馈结果，提示用户确认分析结果；step3.用户确认结果后，提示用户可以进行Ai建模，用户回复后，执行Ai建模，反馈运行结果，提示用户确认模型情况；step4.用户确认模型情况无误后，提示用户可以继续模型波速计算和弹性模量计算，用户回复以后，执行对应的模型参数计算，反馈运行结果，提示用户确认结果\n"
    "当你认为用户意图涉及到调用工具时，判断用户希望执行的工具，必要参数未提供或不完整时，也必须返回tool_calls，前端会渲染表单让用户填写。\n"
    "！当你使用的工具返回了一个图片链接时，你必须将这个链接放在你接下来的回复中，必须显示为图片。\n"
)
REQUIRED_FIELDS = {
    "run_wellloginterpretation": ["query","file_name"],
    "run_GenerateModel": ["query","file_name"],
    "run_ParameterCalc": ["query","file_name"],
    "run_makereport": ["query"],
}
class AppLogger:
    def __init__(self):
